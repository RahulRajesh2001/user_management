...
function EditUser() {
  ...
  const handleSubmit = (e) => {
    ...
    formData.append("profilePhoto", imagePreview);
    ...
    axios
    .put(`${baseUrl}/admin/users/${ID.userId}`, formData, {
      headers: {
  ...
  useEffect(() => {
    const adminToken = localStorage.getItem("adminToken");
    axios
      .get(`${baseUrl}/admin/users/${ID.userId}`, {
        headers: {
          Authorization: adminToken,
        },
      })
  ...
  return (
    <div className="flex items-center justify-center min-h-screen">
      <div className="bg-white p-8 rounded-lg shadow-lg max-w-sm w-full">
        <h2 className="text-2xl font-semibold text-center mb-4">Edit account</h2>
        <form onSubmit={handleSubmit}>
          <div className="flex justify-center mb-6">
            <label htmlFor="upload" className={`inline-block bg-gray-200 rounded-full p-0  cursor-pointer`}>
              <img src={imagePreview ? imagePreview : `${baseUrl}/uploads/${user.profile}`} alt="Preview" className="rounded-full w-32 h-32 object-cover border p-2" />
            </label>
